...
const server=http.createServer((req,res)=>{
  ...
  if(pathname==='/health'){res.writeHead(200,{'content-type':'application/json','cache-control':'no-store'});return res.end(JSON.stringify({ok:true,service:'projeto-daniel-live-connector',clients:wss.clients.size,relay:GameRelay.stats(),signerKey:Boolean(SIGN_API_KEY)}))}
...
wss.on('connection',ws=>{
  ...
  safeSend(ws,{type:'bridge',status:'ready',authRequired:Boolean(ACCESS_KEY),service:'projeto-daniel-live-connector',relay:'websocket-relay-v1'});
  ...
  ws.on('close',()=>{GameRelay.detach(ws);session.disconnect()});
});
