...
wss.on('connection',ws=>{
  ...
  safeSend(ws,{type:'bridge',status:'ready',authRequired:Boolean(ACCESS_KEY),service:'projeto-daniel-live-connector',relay:'websocket-relay-v1',serverAutomation:'liveplus-server-automation-v1'});
  ws.on('message',async raw=>{
    ...
    if(m.type==='server_automation_bind'){const ok=automation.configure(m);return safeSend(ws,{type:'server_automation_status',ok,code:automation.code,ready:automation.ready})}
    if(m.type==='server_automation_sync'){automation.update(m);return safeSend(ws,{type:'server_automation_status',ok:true,code:automation.code,ready:automation.ready})}
