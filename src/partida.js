...
let peer=null,activeConn=null,code='',expiresAt=0,lockedPeer='',sessionToken='',graceUntil=0,timer=null;
...
function render(){
  ...
  if(activeConn?.open)setState('JOGO CONECTADO','busy');else if(peer&&!peer.destroyed)setState('AGUARDANDO JOGO','online');else setState('SESSÃO OFFLINE');
}
function cleanupPeer(){clearInterval(timer);timer=null;try{activeConn?.close()}catch{}try{peer?.destroy()}catch{}activeConn=null;peer=null;lockedPeer='';sessionToken='';graceUntil=0}
...
function acceptConnection(conn){
  ...
  conn.on('open',()=>{expiresAt=0;graceUntil=0;try{conn.send({type:'session_accept',token:sessionToken,exclusive:true,transport:'webrtc',protocol:'liveplus-match-v1'})}catch{}log('Jogo conectado. Código consumido e sessão travada.','ok');render()});
  conn.on('data',data=>{if(!data||typeof data!=='object')return;if(data.type==='session_hello'&&data.token&&data.token!==sessionToken){reject(conn,'TOKEN DE SESSÃO INVÁLIDO');return}if(data.type==='state')window.dispatchEvent(new CustomEvent('liveplus-game-state',{detail:data}));if(data.type==='event')window.dispatchEvent(new CustomEvent('liveplus-game-event',{detail:data}))});
  conn.on('close',()=>{if(activeConn===conn)activeConn=null;graceUntil=Date.now()+RECONNECT_GRACE_MS;log('Jogo desconectou · vaga reservada por 30s para reconexão.','warn');render()});
...
window.LivePlusMatch={start:startSession,end:endSession,getCode:()=>code,send:data=>{if(!activeConn?.open)return false;try{activeConn.send(data);return true}catch{return false}}};
