...
const ACTION_LABELS={spawn:'👾 Criar mob',boss:'👹 Criar Boss',damage:'💥 Dano',heal:'❤️ Cura',freeze:'❄️ Congelar',invincible:'🛡️ Invencível',xp:'✨ XP',level:'⬆️ Level',clear:'🧹 Limpar arena','custom.command':'⚙️ Comando personalizado'};
...
export function renderState(engine,client,els){
  ...
  els.captureToggle.checked=s.settings.capture!==false;els.automationToggle.checked=!!s.settings.automation;els.engineBadge.textContent=s.settings.automation?'AUTOMAÇÃO ON':'CAPTURA';els.catalogCount.textContent=`${verified.length} verificados`;
...
export function renderRules(engine,els){
  els.ruleList.innerHTML=engine.rules.map(r=>`<div class="item"><div class="giftText"><b>${esc(r.trigger)} → <span class="purple">${esc(ACTION_LABELS[r.action]||r.action)}</span></b><small>${r.giftName||r.giftId?`presente ${esc(r.giftName||r.giftId)} · `:''}qtd/limite ${r.quantity} · cooldown ${r.cooldown}s</small></div><button data-delete-rule="${esc(r.id)}">EXCLUIR</button></div>`).join('')||'<div class="notice">Nenhuma automação configurada.</div>';
}
