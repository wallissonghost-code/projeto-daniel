...
export class ConnectorClient extends EventTarget{
  ...
  connect(endpoint,key='',options={}){
    ...
    return new Promise((resolve,reject)=>{let settled=false,opened=false,authSeen=false;const fail=(message,error)=>{this.lastError=message;this.connected=false;this.authenticated=false;this.emit('cloud',{online:false,error:message,recovery});if(!settled){settled=true;clearTimeout(timer);reject(error||new Error(message))}};const timer=setTimeout(()=>fail(opened&&!authSeen?'O conector abriu, mas não respondeu à autenticação.':'Tempo limite ao conectar ao WebSocket.'),7000);let ws;try{ws=new WebSocket(validated)}catch(error){clearTimeout(timer);return fail('Não foi possível abrir o WebSocket.',error)}this.ws=ws;
  ...
  send(payload){if(this.ws?.readyState!==WebSocket.OPEN||!this.connected||!this.authenticated)return false;try{this.ws.send(JSON.stringify(payload));return true}catch{return false}}
