...
export class LiveEngine extends EventTarget{
  ...
  setActiveGame(gameId){
    ...
    const loaded=next?storage.rulesForGame(next):[];
    this.rules=loaded.length?loaded:(next&&storage.profileInitialized(next)?[profileMarker(next)]:[]);
    this.cooldowns.clear();
    this.likeProgress.clear();
    this.emit('rules-profile',{gameId:next,rules:this.rules.filter(r=>!r.__profileMarker),initialized:next?storage.profileInitialized(next):false});
  ...
  saveRule(rule,{source='user'}={}){
    ...
    this.rules=this.rules.filter(r=>!r.__profileMarker);
  ...
  deleteRule(id){if(!this.activeGameId)return;const actual=this.rules.filter(r=>!r.__profileMarker&&r.id!==id);storage.saveRulesForGame(this.activeGameId,actual,{markInitialized:true,userModified:true,source:'user'});this.rules=actual.length?actual:[profileMarker(this.activeGameId)];this.emit('state',this.snapshot())}
  giftMeta(m){const id=m.giftId==null?'':String(m.gift||''),name=String(m.gift||''),g=this.findGift(id,name),unit=Math.max(0,Number(g?.diamondCount)||Number(m.diamondCount)||0),count=Math.max(1,Number(m.count)||1);return{id,name:g?.name||name,count,unit,total:unit*count,verified:Boolean(g)}}
  ...
  match(rule,m){if(rule.__profileMarker||!rule.enabled||String(rule.gameId||'')!==this.activeGameId)return false;if(rule.trigger==='gift'&&m.type==='gift'){const g=this.giftMeta(m);return g.verified&&((rule.giftId&&g.id===String(rule.giftId))||(!rule.giftId&&rule.giftName&&norm(g.name)===norm(rule.giftName)))&&g.count>=rule.quantity}if(rule.trigger==='giftvalue'&&m.type==='gift'){const g=this.giftMeta(m);return g.verified&&g.total>=rule.quantity}if(rule.trigger==='giftany'&&m.type==='gift')return this.giftMeta(m).verified;if(rule.trigger==='like'&&m.type==='like'){const key=rule.id,progress=(this.likeProgress.get(key)||0)+Math.max(1,Number(m.count)||1);this.likeProgress.set(key,progress);return progress>=rule.quantity}if(rule.trigger==='chat'&&m.type==='chat'){const wanted=norm(rule.commentText);return !wanted||wanted==='*'||norm(m.comment).includes(wanted)}return rule.trigger===m.type}
  runRules(m){const now=Date.now();for(const rule of this.rules){if(!this.match(rule,m)||!this.canFire(rule))continue;if(rule.trigger==='like')this.likeProgress.set(rule.id,Math.max(0,(this.likeProgress.get(rule.id)||0)-rule.quantity));const gift=m.type==='gift'?this.giftMeta(m):null;const traceId=String(m.traceId||'');const trace={traceId,connectorReceivedAt:Number(m.connectorReceivedAt)||0,connectorSentAt:Number(m.connectorSentAt)||0,panelReceivedAt:Number(m.panelReceivedAt)||now};const payload={type:'live.rule',version:2,traceId,trace,ruleId:rule.id,trigger:rule.trigger,threshold:rule.quantity,commentText:rule.commentText||'',game:{id:rule.gameId||'',name:rule.gameName||''},action:{id:rule.actionId||'',label:rule.actionLabel||'',params:rule.actionParams||{}},event:{type:m.type,user:m.user||'',comment:m.comment||'',gift:gift?.name||m.gift||null,giftId:m.giftId||null,count:m.count||1,diamondValue:gift?.unit||0,totalDiamonds:gift?.total||0,verifiedGift:gift?.verified??null,at:now,traceId,...trace}};this.client.emitAction('live.rule',payload,rule.id);if(!globalThis.LivePlusServerAutomation?.active?.())this.emit('automation',{rule,event:m,payload})}}
}
