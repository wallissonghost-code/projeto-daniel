...
export class LiveEngine extends EventTarget{
  constructor(client){
    super();this.client=client;this.settings=storage.settings();this.catalog=[];this.rules=storage.rules();
  ...
  snapshot(){return {settings:this.settings,catalog:this.catalog,rules:this.rules,stats:{...this.stats}}}
  ...
  setMasterCatalog(gifts=[]){
    ...
    for(const raw of gifts){
      const id=raw.id==null?'':String(raw.id),name=String(raw.name||'Presente'),key=id||norm(name);
      if(!key)continue;
      map.set(key,{...raw,id:id||null,name,diamondCount:Math.max(0,Number(raw.diamondCount)||0),icon:String(raw.icon||''),masterVerified:true,liveVerified:true,liveDivergence:false,verifiedAt:Number(raw.verifiedAt)||Date.now()});
    }
    this.catalog=[...map.values()].sort((a,b)=>(a.diamondCount-b.diamondCount)||a.name.localeCompare(b.name));
    this.emit('catalog',this.catalog);this.emit('state',this.snapshot());
  }
  onMessage(m){
    ...
    if(['like','chat','follow','share','gift'].includes(m.type)&&!unfinishedStreak){
      ...
      if(this.settings.automation)this.runRules(m);
    }
  ...
  match(rule,m){
    ...
    if(rule.trigger==='gift'&&m.type==='gift'){const g=this.giftMeta(m);return g.verified&&((rule.giftId&&g.id===String(rule.giftId))||(!rule.giftId&&rule.giftName&&norm(g.name)===norm(rule.giftName)))&&g.count>=rule.quantity}
