...
function migrateGiftReferences(catalog=[]){
  const gifts=Array.isArray(catalog)?catalog:[];const byId=new Map(),byName=new Map();
  ...
  for(const [gameId,rules] of Object.entries(all)){
    if(!Array.isArray(rules)||gameId===UNSCOPED)continue;let profileChanged=false;
    const next=rules.map(rule=>{if(!rule||rule.__profileMarker||rule.trigger!=='gift')return rule;const oldId=rule.giftId==null?'':String(rule.giftId),oldName=norm(rule.giftName);let gift=oldId?byId.get(oldId):null;if(!gift&&oldName){const matches=byName.get(oldName)||[];if(matches.length===1)gift=matches[0]}if(!gift)return rule;const newId=gift.id==null?'':String(gift.id),newName=String(gift.name||rule.giftName||''),newIcon=String(gift.icon||''),newValue=Math.max(0,Number(gift.diamondCount)||0);const changed=oldId!==newId||String(rule.giftName||'')!==newName||String(rule.giftIcon||'')!==newIcon||Number(rule.giftValue||0)!==newValue;if(!changed)return rule;profileChanged=true;changedRules++;return{...rule,giftId:newId,giftName:newName,giftIcon:newIcon,giftValue:newValue,catalogMigratedAt:Date.now(),catalogPreviousGiftId:oldId&&oldId!==newId?oldId:(rule.catalogPreviousGiftId||'')}});
...
function managedSettings(){const saved=read(keys.settings,{});return{...saved,endpoint:MANAGED_CONNECTOR_ENDPOINT,key:MANAGED_SESSION_MARKER,username:String(saved?.username||''),capture:saved?.capture!==false,automation:!!saved?.automation}}
function saveManagedSettings(v={}){const current=read(keys.settings,{});const {endpoint,key,...safe}=v;return write(keys.settings,{...current,...safe,endpoint:'',key:''})}
