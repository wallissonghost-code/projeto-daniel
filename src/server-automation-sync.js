...
const state={bound:false,ready:false,game:false,code:'',endpoint:'',catalog:[],lastSync:''};
...
function bindIfPossible(){const relay=window.LivePlusServerRelay,match=window.LivePlusMatch;if(!relay||!match)return false;const rs=relay.state?.()||{},code=String(match.getCode?.()||'').trim().toUpperCase(),endpoint=String(rs.endpoint||relay.endpoint?.()||'').trim();if(!code||!endpoint||!rs.game)return false;state.code=code;state.endpoint=endpoint;const payload={type:'server_automation_bind',code,endpoint,key:relayKey(),rules:readRules(),catalog:state.catalog};relay.leave?.();state.bound=true;state.ready=false;state.game=false;send(payload);return true}
function sync(){if(!state.bound)return bindIfPossible();const rules=readRules(),signature=JSON.stringify(rules);if(signature!==state.lastSync){state.lastSync=signature;send({type:'server_automation_sync',rules,catalog:state.catalog})}return true}
...
loadCatalog().then(()=>setTimeout(sync,100));
