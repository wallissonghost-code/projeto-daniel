...
els.testPanel.onclick=async()=>{
  const originalAutomation=engine.settings.automation,originalCapture=engine.settings.capture;
  engine.saveSettings({capture:true,automation:true});engine.resetSession();
  ...
  const samples=[
    ...
    {type:'gift',user:'nexus.qa',gift:'Rose',giftId:'5655',diamondCount:1,count:2,icon:''}
  ];
  ...
  engine.saveSettings({automation:originalAutomation,capture:originalCapture});
};
