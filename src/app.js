...
els.connectCloud.onclick=async()=>{persist();try{await client.connect(els.endpoint.value.trim(),els.accessKey.value);els.connectorNotice.textContent='Cloud conectado.'}catch{els.connectorNotice.textContent='Não foi possível conectar ao Cloud.'}redraw()};
