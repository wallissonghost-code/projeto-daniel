...
function durationLabel(){return els.duration?.closest('.stat')?.querySelector('span')||null}
function formatElapsed(started){const sec=Math.max(0,Math.floor((Date.now()-started)/1000)),m=Math.floor(sec/60),s=sec%60;return `${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`}
function finishLocalLiveSession(){const started=engine.stats.startedAt;if(started)els.duration.textContent=formatElapsed(started);engine.stats.startedAt=0;engine.stats.last=null;const label=durationLabel();if(label)label.textContent='ÚLTIMA SESSÃO';setLiveStatus(els,{status:'disconnected',reason:'Sessão encerrada'});els.healthEvents.textContent='AGUARDANDO';engine.emit('state',engine.snapshot())}
function startLocalLiveSession(){const label=durationLabel();if(label)label.textContent='DURAÇÃO';els.duration.textContent='00:00'}
...
els.stopLive.onclick=()=>{if(!requireConnector())return;client.stopLive();finishLocalLiveSession();notice('Comando para parar a Live enviado.')};
